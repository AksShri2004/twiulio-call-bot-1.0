...
def load_twilio_numbers():
    local_json_path = os.path.join(os.getcwd(), 'twilio_numbers.json')

    # Check if a local twilio_numbers.json file exists
    if os.path.exists(local_json_path):
        with open(local_json_path, 'r') as file:
            data = json.load(file)
            return data['twilio_numbers']
    else:
        # If no local file, load the bundled default file
        ...
        if os.path.exists(json_path):
            ...
            # Default Twilio numbers if JSON file doesn't exist
            return [
                #numbers active on twilio
            ]


# Function to save Twilio numbers to the local JSON file
def save_twilio_numbers():
    ...
    local_json_path = os.path.join(os.getcwd(), 'twilio_numbers.json')  # Save to local directory
    with open(local_json_path, 'w') as file:
        json.dump(data, file, indent=4)

    update_status("Twilio numbers saved to twilio_numbers.json in the local directory")
